...
\title{Smart Compaction: Predicting Compaction Utility from Lakehouse Table Metadata\textsuperscript{$\star$}%
\thanks{$\star$The views expressed in this paper are solely those of the authors and do not necessarily reflect the views of their respective institutions or employers. All code and data for full replication are available at \url{https://github.com/JannicCutura/smart_compaction}.}
}
...
\IEEEauthorblockA{\textsc{Nordcloud}\textsuperscript{\S}\\
{\small\color{gray!80!black}\href{mailto:subash.prakash@nordcloud.com}{\texttt{subash.prakash@nordcloud.com}}}}
}
...
\footnotetext[4]{Nordcloud (an IBM Company), Haid-und-Neu-Stra\ss e 18, 76131 Karlsruhe, Germany.}
...
\begin{IEEEkeywords}
data lakehouse, open table formats, compaction, machine learning, table optimization
\end{IEEEkeywords}
...
Hudi~\cite{hudi_compaction}. These formats use append-only, log-structured
write patterns where each transaction appends immutable columnar files,
providing snapshot isolation at the cost of progressive file accumulation.

This accumulation leads to the \emph{small file problem}: the number of data
files grows without bound, increasing query planning latency, inflating
per-request costs on cloud object stores, and degrading scan throughput. At
scale, catalogs with thousands of tables can accumulate millions of undersized
files~\cite{gruenheid2025autocomp}. \emph{Compaction} addresses this by
rewriting small files into fewer target-sized files via bin-packing, sort, or
z-order strategies. However, compaction itself is costly in terms of compute and
I/O, so the question is not \emph{whether} to compact but \emph{when} the
file reduction justifies the cost.
...
LSM-tree compaction research has established cost models for merge
policies~\cite{dayan2017monkey, dayan2018dostoevsky}, quantified the
...
stores without a level hierarchy, where file proliferation and scan
overhead rather than write amplification drive costs. We share the
core question --- \emph{when is reorganizing stored data worth the
compute cost?} --- but address it with metadata-driven prediction
rather than policy optimization.
...
OtterTune~\cite{vanaken2017ottertune} (Gaussian Process regression) and
CDBTune~\cite{zhang2019cdbtune} (deep RL) to
NoisePage's~\cite{pavlo2021noisepage} forecast--model--plan framework. Our work
instantiates this pattern: metadata features forecast compaction utility, and
the binary decision constitutes the action plan.

In the broader ``ML for systems'' movement~\cite{zhou2022dbmeetsai},
Bao~\cite{marcus2021bao} steers query optimization via learned hint
selection, directly analogous to our approach of steering compaction via a
learned trigger. Qd-tree~\cite{yang2020qdtree} learns workload-aware data
layouts. Despite this progress, no published work applies ML to predict the
utility of a storage maintenance operation in the lakehouse context.
AutoComp~\cite{gruenheid2025autocomp} is the closest predecessor, but
relies on heuristic trait computation rather than learned models.
...
because the heterogeneous batch pattern (Section~II-B2) uses an $8\times$
...
\begin{figure}[!ht]
\centerline{\includegraphics[width=\columnwidth]{plots/feature_violins.pdf}}
...
\begin{figure}[b]
\centerline{\includegraphics[width=\columnwidth]{plots/feature_importance_clf.pdf}}
...
\begin{figure}[b]
\centerline{\includegraphics[width=\columnwidth]{plots/regression_scatter.pdf}}
...
\begin{figure}[tb]
\centerline{\includegraphics[width=\columnwidth]{plots/feature_ablation.pdf}}
...
Section~III-B: partition-level file counts carry nearly all the
...
tree-based model suffices. Logistic regression performs poorly
($R^2 = 0.361$), underscoring the nonlinear structure of the
regression target.
...
Logistic Reg.     & 0.815 & 0.885 & 0.361 & 0.236 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[b]
\centerline{\includegraphics[width=\columnwidth]{plots/query_speedup.pdf}}
...
target-leakage concern. However, this feature ranks outside the top-5
by gain importance (\texttt{max\_files\_per\_partition} and
\texttt{avg\_files\_per\_partition} account for 95.2\% of regressor
gain), and the leave-one-out ablation (Section~\ref{sec:ablation})
shows that removing it does not degrade $R^2$ below 0.988. The leakage
...
Our dataset provides empirical guidance: of the 2{,}126 tables that
